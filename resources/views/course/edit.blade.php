<x-app-layout>
                ...
                <div class="bg-white shadow-md rounded my-6 p-5">
                    <form method="POST" action="{{ route('admin.courses.update', $course->id) }}"
                        ...
                        <div class="flex flex-col space-y-2 mt-2">
                            ...
                            <input id="c_name" type="text" name="c_name" value="{{$course->c_name}}"
                                placeholder="Course Name"
                                class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200" />
                        </div>
                        <div class="flex flex-col space-y-2 mt-2">
                            <label for="title" class="text-gray-700 select-none font-medium">Short Description</label>
                            <input id="c_detail" type="text" name="c_detail" value="{{$course->c_detail}}"
                                placeholder="Detail"
                                class="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-200" />
                        </div>
                        <div class="flex flex-col space-y-2 mt-2">
                            <label for="title" class="text-gray-700 select-none font-medium">Price</label>
                            <input id="c_price" type="number" name="c_price" value="{{$course->c_price}}"
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            <div class="relative inline-flex">
                                ...
                                <select
                                    ...
                                    name="c_status">
                                    <option value="0" @if($course->c_status == 0) selected @endif>Deactive</option>
                                    <option value="1" @if($course->c_status == 1) selected @endif>Active</option>
                                </select>
                            </div>
                        </div>
    ...
    <script>
        function imageData3() {
            ...
            if (files.length == 0) {
                var url = '/courseimages/' + {!! json_encode($course->c_img) !!};
            } else {
                url = '';
            }
            return {
